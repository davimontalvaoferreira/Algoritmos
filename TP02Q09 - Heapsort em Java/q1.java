...
public class q1 {
	...
	public static void constroi(int tamHeap) {
		try {
			for (int i = tamHeap; i > 1 && array[i].getRelease_date().getTime() > array[i / 2].getRelease_date().getTime() ; i /= 2) {
				swap(i, i / 2);
			}
		} catch (Exception e) {
	...
	public static void reconstroi(int tamHeap) {
		...
		while (i <= (tamHeap / 2)) {
			try {
				if (array[2 * i].getRelease_date().getTime()  > array[2 * i + 1].getRelease_date().getTime()  || 2 * i == tamHeap) {
					filho = 2 * i;
				} else {
					filho = 2 * i + 1;
				}
	
				if (array[i].getRelease_date().getTime() < array[filho].getRelease_date().getTime() ) {
					swap(i, filho);
					i = filho;
				} else {
					i = tamHeap;
