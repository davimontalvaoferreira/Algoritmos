...
public class q1 {
	...
	public static void main (String[] args) throws ParseException {
		...
		String[] entradaInsercao = new String[200000]; //valor adequeado pra nao dar overflow
		String[] entradaPesquisa = new String[512];
		String[] datacsv= new String [170627];

		Musica per = new Musica();
        ...
        do {
            entradaInsercao[numEntrada] = MyIO.readLine();
        } while (entradaInsercao[numEntrada++].equals("FIM") == false);
        numEntrada--; // Desconsiderar ultima linha contendo a palavra FIM

		do {
            entradaPesquisa[numEntrada2] = MyIO.readLine();
        } while (entradaPesquisa[numEntrada2++].equals("FIM") == false);
        numEntrada2--; // Desconsiderar ultima linha contendo a palavra FIM

        // Para cada entrada pesquisar atributos
        // for (int i = 0; i < numEntrada; i++) {
		// 	per.ler(entradaInsercao[i], datacsv);  
		// 	vetor[i] = per.getId();
		// 	//MyIO.println(""+i);      
        // }
		...
		// 	pesquisar(entradaPesquisa, vetor[i], numEntrada2);
		// }

		boolean temid = false;
		for (int i = 0; i < numEntrada2; i++) {
			for (int j = 0; j < numEntrada; j++) {
				if(entradaPesquisa[i].equals(entradaInsercao[j])){
					temid = true;
					j = numEntrada;
				}
			}
